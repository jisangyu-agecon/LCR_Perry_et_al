...
local gs apr_sep
use ddayreg_89_2014_`gs'_aph.dta
...
preserve
keep if cropcode==41
outreg2 using summary_corn_aph.tex, replace sum(log) keep(loss_lia cause_10_loss_lia cause_12_loss_lia cause_25_loss_lia cold_related_loss_lia cause_26_loss_lia)
restore
...
outreg2 using warming_corn_aph.tex, replace
...
outreg2 using warming_corn_aph.tex, append
...
preserve
keep if cropcode==81
outreg2 using summary_soy_aph.tex, replace sum(log) keep(loss_lia cause_10_loss_lia cause_12_loss_lia cause_25_loss_lia cold_related_loss_lia cause_26_loss_lia)
...
(DDsecond_loss_lia_1-DDsecond_loss_lia)*_b[DDsecond_loss_lia]+ ///
(DDthird_loss_lia_1-DDthird_loss_lia)*_b[DDthird_loss_lia], post
outreg2 using warming_soy_aph.tex, replace
...
(DDsecond_d_1-DDsecond_d)*_b[DDsecond_d]+ ///
(DDthird_d_1-DDthird_d)*_b[DDthird_d], post
outreg2 using warming_soy_aph.tex, append
